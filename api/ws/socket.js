...
module.exports = function expressWs(app, httpServer, options = {}) {
  ...
  const me = {
    ...
    broadcast: (
      ...
      wsServer.clients.forEach(client => {
        ...
        if (options.skipSelf)
          shouldBroadcastToThisClient =
            shouldBroadcastToThisClient && client !== currentClient;

        if (currentClient.room  && !options.allClients)
          shouldBroadcastToThisClient =
            shouldBroadcastToThisClient && client.room === currentClient.room;

        const theSocketIsOpen =
          currentClient.readyState === 1; /* WebSocket.OPEN */
   ...
   * object. */
  if (!options.leaveRouterUntouched) addWsMethod(express.Router, me);

  wsServer.on("connection", (socket, request) => {
    if ("upgradeReq" in socket) request = socket.upgradeReq;
